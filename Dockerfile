...
RUN  mkdir /opt/gatling/bin
RUN  mkdir /opt/gatling/conf
RUN  mkdir /opt/gatling/results
RUN  mkdir /opt/gatling/user-files
...
COPY entrypoint/executor_executable_jar.sh /tmp
RUN sudo chmod +x /tmp/executor_executable_jar.sh
...
#COPY jar_files/gatling-http-3.7.6.jar /opt/gatling/lib
#COPY jar_files/gatling-core-3.7.6.jar /opt/gatling/lib
...
ENTRYPOINT ["/tmp/executor_executable_jar.sh"]