...
sealed abstract class StatsProcessor extends StrictLogging {
  ...
  private def logTx(
    ...
    //    var build_id = System.getProperty("build_id", "")
    //    if (build_id.isEmpty) build_id = System.getenv("build_id")
    //    if (build_id == null) build_id = getPropertyFromFile("build_id")
    val build_id = getProperty("build_id")

    //    val lg_id = System.getProperty("lg_id", "")
    val lg_id = getProperty("lg_id")
    val user_id = session.userId
    //    val test_type = System.getProperty("test_type", "")
    val test_type = getProperty("test_type")
    //    val simulation_name = System.getProperty("simulation_name", "")
